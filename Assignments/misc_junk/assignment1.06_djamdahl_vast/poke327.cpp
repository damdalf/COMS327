...
void new_hiker()
{
  ...
  // TODO
  c = new NPC(char_hiker, move_hiker, 0, pos, 'h', 0);
...
void new_rival()
{
  ...
  // TODO
  c = new NPC(char_rival, move_rival, 0, pos, 'r', 0);
  world.cur_map->cmap[pos[dim_y]][pos[dim_x]] = c;
  heap_insert(&world.cur_map->turn, c);
}
...
void new_char_other()
{
  ...
  // TODO
  // world.cur_map->cmap[pos[dim_y]][pos[dim_x]] = c = new Character();
  // c->npc = new NPC();
  ...
  switch (rand() % 4) {
  case 0:
    c = new NPC(char_other, move_pace, 0, pos, 'p', 0);
    world.cur_map->cmap[pos[dim_y]][pos[dim_x]] = c;
    // c->npc->mtype = move_pace;
    // c->symbol = 'p';
    break;
  case 1:
    c = new NPC(char_other, move_wander, 0, pos, 'w', 0);
    world.cur_map->cmap[pos[dim_y]][pos[dim_x]] = c;
    // c->npc->mtype = move_wander;
    // c->symbol = 'w';
    break;
  case 2:
    c = new NPC(char_other, move_sentry, 0, pos, 's', 0);
    world.cur_map->cmap[pos[dim_y]][pos[dim_x]] = c;
    // c->npc->mtype = move_sentry;
    // c->symbol = 's';
    break;
  case 3:
    c = new NPC(char_other, move_walk, 0, pos, 'n', 0);
    world.cur_map->cmap[pos[dim_y]][pos[dim_x]] = c;
...
void game_loop()
{
  ...
  while (!world.quit) {
    ...
    // TODO changed from checking if pc or npc is not null, checking type since all character and sub classes have the member
...
int main(int argc, char *argv[])
{
  ...
  if (argc == 2) 
  {
    ...
    {
      vector<Type_Name> typeNamesList;
