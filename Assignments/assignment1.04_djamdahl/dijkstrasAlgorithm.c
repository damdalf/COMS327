...
void dijkstraTrainer(map_t *m, int k)
{
    int i, j;
...
void dijkstraTrainers(map_t *m, const int NUM_TRAINERS)
{
    int i, j, k;
...
void findNextMoveHikersAndRivals(map_t *m, int k, const int NUM_TRAINERS)
{
    ...
    {
        // Check for all possible NPCs.
        for (l = 0; l < NUM_TRAINERS; l++)
        {
            // Locate the NPC that is in said position.
            if (m->trainers[l]->xPos == i && m->trainers[l]->yPos == j)
            {
                // If terrain cell is occupied by another NPC, set the cost of that cell to infinity.
...
void findNextMovePacer(map_t *m, int k)
{
        ...
        case 0: // N
            if (i - 1 > 0)
            {
                if (m->mapArray[i - 1][j] != BORDER && m->mapArray[i - 1][j] != TREE && m->mapArray[i - 1][j] != ROCK && m->mapArray[i - 1][j] != MART
                    && m->mapArray[i - 1][j] != CENTER && m->mapArray[i - 1][j] != PLAYER && m->characterArray[i - 1][j] == VACANT)
                {
                    i -= 1;
                }
                else
        ...
        case 1: // NE
            if (i - 1 > 0 && j + 1 < WIDTH - 1)
            {
                if (m->mapArray[i - 1][j + 1] != BORDER && m->mapArray[i - 1][j + 1] != TREE && m->mapArray[i - 1][j + 1] != ROCK && m->mapArray[i - 1][j + 1] != MART
                    && m->mapArray[i - 1][j + 1] != CENTER && m->mapArray[i - 1][j + 1] != PLAYER && m->characterArray[i - 1][j + 1] == VACANT)
                {
                    i -= 1;
                    j += 1;
                }
        ...
        case 2: // E
            if (j + 1 < WIDTH - 1)
            {
                if (m->mapArray[i][j + 1] != BORDER && m->mapArray[i][j + 1] != TREE && m->mapArray[i][j + 1] != ROCK && m->mapArray[i][j + 1] != MART
                    && m->mapArray[i][j + 1] != CENTER && m->mapArray[i][j + 1] != PLAYER && m->characterArray[i][j + 1] == VACANT)
                {
                    j += 1;
                }
                else
    ...
    else if (i < 20 && j < 79 && i > 0 && j > 0 && m->characterArray[i][j] != VACANT) // TODO: SEEMS UN-NEEDED
    {
        switch(m->trainers[k]->direction)
        {
            case 0:
...
void findNextMoveWanderer(map_t *m, int k)
{
    ...
    switch (m->trainers[k]->direction) {
        case 0: // N
            if (i - 1 > 0) {
                if (m->mapArray[i - 1][j] != m->trainers[k]->previousTerrain || m->characterArray[i - 1][j] != VACANT) {
        ...
        case 1: // NE
            if (i - 1 > 0 && j + 1 < WIDTH - 1) {
                if (m->mapArray[i - 1][j + 1] != m->trainers[k]->previousTerrain || m->characterArray[i - 1][j + 1] != VACANT) {
                    generateDirections(m, k);
                    findNextMoveWanderer(m, k);
                } else {
                    i -= 1;
        ...
        case 2: // E
            if (j + 1 < WIDTH - 1) {
                if (m->mapArray[i][j + 1] != m->trainers[k]->previousTerrain || m->characterArray[i][j + 1] != VACANT) {
                    generateDirections(m, k);
                    findNextMoveWanderer(m, k);
                } else {
                    j += 1;
...
void findNextMoveRandomWalker(map_t *m, int k)
{
    ...
    switch (m->trainers[k]->direction) {
        case 0: // N
            ...
            {
                if (m->mapArray[i - 1][j] != BORDER && m->mapArray[i - 1][j] != TREE && m->mapArray[i - 1][j] != ROCK && m->mapArray[i - 1][j] != MART
                    && m->mapArray[i - 1][j] != CENTER && m->mapArray[i - 1][j] != PLAYER && m->characterArray[i - 1][j - 1] == VACANT)
                {
                    i -= 1;
                }
                else
        ...
        case 2: // E
            ...
            {
                if (m->mapArray[i][j + 1] != BORDER && m->mapArray[i][j + 1] != TREE && m->mapArray[i][j + 1] != ROCK && m->mapArray[i][j + 1] != MART
                    && m->mapArray[i][j + 1] != CENTER && m->mapArray[i][j + 1] != PLAYER && m->characterArray[i - 1][j - 1] == VACANT)
                {
                    j += 1;
                }
                else
...
void updateTrainerLocations(map_t*m, const int NUM_TRAINERS)
{
    ...
    for (k = 0; k < NUM_TRAINERS; k++)
    {
        ...
        if (m->trainers[k]->symbol != STATIONARIES)
        {
                ...
                {
                    case HIKERS:
                        findNextMoveHikersAndRivals(m, k, NUM_TRAINERS);
                        break;
                    case RIVALS:
                        findNextMoveHikersAndRivals(m, k, NUM_TRAINERS);
                    break;
    ...
    dijkstraPC(m);
    dijkstraTrainers(m, NUM_TRAINERS);
}
