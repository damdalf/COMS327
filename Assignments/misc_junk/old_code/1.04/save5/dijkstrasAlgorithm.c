...
void findNextMoveHikersAndRivals(map_t *m, const int NUM_TRAINERS)
{
    ...
    for (k = 0; k < NUM_TRAINERS; k++)
    {
        if (m->trainers[k]->symbol == HIKERS || m->trainers[k]->symbol == RIVALS)
        {
            ...
            if (i - 1 >= 0 && i + 1 <= 19 && j - 1 >= 0 && j + 1 <= 78)
            {
                // N
                if (m->characterArray[i - 1][j] == VACANT) directions[0] = m->trainers[k]->costMap[i - 1][j].priority;
                else directions[0] = INT_MAX;
                // NE
                if (m->characterArray[i -1 ][j + 1] == VACANT) directions[1] = m->trainers[k]->costMap[i - 1][j + 1].priority;
                else directions[1] = INT_MAX;
                // E
                if (m->characterArray[i][j + 1] == VACANT) directions[2] = m->trainers[k]->costMap[i][j + 1].priority;
                else directions[2] = INT_MAX;
                // SE
                if (m->characterArray[i + 1][j + 1] == VACANT) directions[3] = m->trainers[k]->costMap[i + 1][j + 1].priority;
                else directions[3] = INT_MAX;
                // S
                if (m->characterArray[i + 1][j] == VACANT) directions[4] = m->trainers[k]->costMap[i + 1][j].priority;
                else directions[4] = INT_MAX;
                // SW
                if (m->characterArray[i + 1][j - 1] == VACANT) directions[5] = m->trainers[k]->costMap[i + 1][j - 1].priority;
                else directions[5] = INT_MAX;
                // W
                if (m->characterArray[i][j - 1] == VACANT) directions[6] = m->trainers[k]->costMap[i][j - 1].priority;
                else directions[6] = INT_MAX;
                // NW
                if (m->characterArray[i + 1][j - 1] == VACANT) directions[7] = m->trainers[k]->costMap[i + 1][j - 1].priority;
                else directions[7] = INT_MAX;
            }
            ...
            if (m->trainers[k]->atPC == false)
            {
                ...
                if (m->mapArray[i][j] == PLAYER || m->characterArray[i][j] != VACANT) //TODO after more testing, might be able to only check via characterArray.
                {
                    occupied = true;
                }
                else
            ...
            switch(m->trainers[k]->direction)
            {
                case 0: // N
                    if (i - 1 >= 0)
                    {
                        ...
                        && m->mapArray[i - 1][j] != CENTER && m->characterArray[i - 1][j] == VACANT)
                        {
                            i -= 1;
                        }
                        else
                ...
                case 1: // NE
                    if (i - 1 >= 0 && j + 1 <= 78)
                    {
                        if (m->mapArray[i - 1][j + 1] != BORDER && m->mapArray[i - 1][j + 1] != TREE && m->mapArray[i - 1][j + 1] != ROCK && m->mapArray[i - 1][j + 1] != MART
                            && m->mapArray[i - 1][j + 1] != CENTER && m->characterArray[i - 1][j + 1] == VACANT)
                        {
                            i -= 1;
                            j += 1;
                        }
                ...
                case 2: // E
                    if (j + 1 <= 78)
                    {
                        if (m->mapArray[i][j + 1] != BORDER && m->mapArray[i][j + 1] != TREE && m->mapArray[i][j + 1] != ROCK && m->mapArray[i][j + 1] != MART
                            && m->mapArray[i][j + 1] != CENTER && m->characterArray[i][j + 1] == VACANT)
                        {
                            j += 1;
                        }
                        else
...
void updateTrainerLocations(map_t*m, const int NUM_TRAINERS)
{
    ...
    // TODO: NOT NEEDED FOR THIS ASSIGNMENT AS THE PLAYER DOES NOT MOVE, THUS THE COST MAP SHALL REMAIN CONSTANT.
    // Initialize cost maps.
    //initializeCostMaps(m, NUM_TRAINERS);
    // TODO: NOT NEEDED FOR THIS ASSIGNMENT AS THE PLAYER IS NOT REQUIRED TO BE ABLE TO MOVE.
    ...
    // TODO: TRYING TO ONLY FIND THE NEXT MOVE OF CHARACTERS THAT IS NECESSARY
    ...
    for (k = 0; k < NUM_TRAINERS; k++)
    {
        if (m->trainers[k]->symbol != STATIONARIES)
        {
            ...
            if (m->trainers[k]->atPC == true)
            {
                // if the NPC, no more moves for the particular character need to be made.
                continue;
            }
            else
            {
