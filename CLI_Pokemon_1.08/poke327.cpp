...
void new_hiker()
{
  ...
  world.cur_map->cmap[pos[dim_y]][pos[dim_x]] = n = new Npc;
  n->pos[dim_y] = pos[dim_y];
  n->pos[dim_x] = pos[dim_x];
  n->ctype = char_hiker;
  n->mtype = move_hiker;
  n->dir[dim_x] = 0;
  n->dir[dim_y] = 0;
  ...
  n->symbol = 'h';
  n->next_turn = 0;
  Character *c = dynamic_cast< Character*> (n);
  generate_starting_pokemon(c);
...
void game_loop()
{
  ...
  Pc *p;
  pair_t d;
  // bool random_p_generated = false;
  
  while (!world.quit) {
    // remove character from the heap
    c = (Character *) heap_remove_min(&world.cur_map->turn);
    // dynamic cast to later check for type
    n = dynamic_cast<Npc *> (c);
    p = dynamic_cast<Pc *> (c);

    // check if character is pc and if random pokemon have been generated
    // if (p && !random_p_generated)
    // {
    //   generate_starting_pokemon(c);
    //   random_p_generated = true;
    // }
    ...
    // check if character is PC and if exiting map
    ...
    // calculate value of character's next move
    c->next_turn += move_cost[n ? n->ctype : char_pc]
                             [world.cur_map->map[d[dim_y]][d[dim_x]]];

    // check if PC is on grass, and deal with wild pokemon encounter
    ...
    // insert character back into the heap with new informaiton
    heap_insert(&world.cur_map->turn, c);
  }
}
...
int main(int argc, char *argv[])
{
  ...
  // check if the user has entered a specific seed, otherwise generate randomly
  if (argc == 2) {
    seed = atoi(argv[1]);
  } else {
  ...
  // get the terminalr ready for ncurses
  io_init_terminal();
  // set this equal to true. could be wrong, but would assume that we need to use this function to complete this assignment
  db_parse(false);
  // initialize the world 
  init_world();
  // generate starting pokemon for pc
  Character* c = dynamic_cast<Character *> (&world.pc);
  generate_starting_pokemon(c);
  // initialize amount of items for the pc
  world.pc.bag[0] = 12; // revives
  world.pc.bag[1] = 12; // potions
  world.pc.bag[2] = 5; // Pokeballs
  // begin the game loop, within here is where we will want to handle the generation of starting pokemon
  game_loop();
  // when game loop is exited, we want to delete all memory associated with the game
  delete_world();
  // before finally exiting, reset the terminal to normal condiditons (bye ncurses)
  io_reset_terminal();
