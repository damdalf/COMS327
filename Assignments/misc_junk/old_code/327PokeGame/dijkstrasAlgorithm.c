        ...
        if (m->trainers[k]->symbol == PACERS)
        {
            ...
            switch(m->trainers[k]->direction)
            {
                case 0: // N
                    if (i - 1 > 0)
                    {
                        if (m->trainers[k]->costMap[i - 1][j].priority == INT_MAX)
                        {
                ...
                case 1: // NE
                    if (i - 1 > 0 && j + 1 < 20)
                    {
                        if (m->trainers[k]->costMap[i - 1][j + 1].priority == INT_MAX)
                        {
                ...
                case 2: // E
                    if (j + 1 < 20)
                    {
                        if (m->trainers[k]->costMap[i][j + 1].priority == INT_MAX)
                        {
            ...
            printf("PACER NO. %d'S CURRENT DIRECTION: %d\n", k + 1, m->trainers[k]->direction);
            printf("PACER NO. %d'S CURRENT LOCATION: X = %d, Y = %d\n", k + 1, m->trainers[k]->yPos, m->trainers[k]->xPos);

            // TODO chang checking occupancy to use char values
            /*
             * for (l = 0; l < NUM_TRAINERS; l++)
                {
                    if (m->trainers[l]->xPos != i && m->trainers[l]->yPos != j && l != k) occupied = false;
                    else occupied = true;
                }
                // Now check for the PC.
                if (i != m->pc.xPos && j != m->pc.yPos && occupied != true) occupied = false;
            ...
            // Ensure that new position is not already occupied by a Trainer, and that the Trainer has not changed direction.
            if (!changedDirection)
            {
                if (m->mapArray[i][j] == PLAYER || m->mapArray[i][j] == HIKERS || m->mapArray[i][j] == RIVALS || m->mapArray[i][j] == PACERS || m->mapArray[i][j] == WANDERERS || m->mapArray[i][j] == RANDOM_WALKER || m->mapArray[i][j] == STATIONARIES)
                {
                    occupied = true;
                }
                ...
                printf("PACER NO. %d'S NEXT MOVE: X = %d, Y = %d\n\n", k + 1, j, i);
            }
            else
            {
                printf("PACER NO. %d CHANGED DIRECTION.\n\n", k + 1);
            }
        }
    }
    printf("\n");
...
void findNextMoveWanderers(map_t *m, const int NUM_TRAINERS)
{
    ...
    for (k = 0; k < NUM_TRAINERS; k++)
    {
        if (m->trainers[k]->symbol == WANDERERS)
        {
            ...
            printf("WANDERER NO. %d'S CURRENT DIRECTION: %d\n", k + 1, m->trainers[k]->direction);
            printf("WANDERER NO. %d'S CURRENT LOCATION: X = %d, Y = %d\n", k + 1, m->trainers[k]->yPos, m->trainers[k]->xPos);
            ...
            if (!changedDirection)
            {
                ...
                // Now check for the PC.
                if (i != m->pc.xPos && j != m->pc.yPos && occupied != true && l != k) occupied = false;
                ...
                //Just to test, print the coordinates of the Pacer's next move.
                printf("WANDERER NO. %d'S NEXT MOVE: X = %d, Y = %d\n\n", k + 1, j, i);
            }
            else
            {
                printf("WANDERER NO. %d CHANGED DIRECTION.\n\n", k + 1);
            }
        }
    }
    printf("\n");
...
void findNextMoveRandomWalkers(map_t *m, const int NUM_TRAINERS)
{
    ...
    for (k = 0; k < NUM_TRAINERS; k++)
    {
        if (m->trainers[k]->symbol == RANDOM_WALKER) {
            ...
            printf("WALKER NO. %d'S CURRENT DIRECTION: %d\n", k + 1, m->trainers[k]->direction);
            printf("WALKER NO. %d'S CURRENT LOCATION: X = %d, Y = %d\n", k + 1, m->trainers[k]->yPos, m->trainers[k]->xPos);
            ...
            if (!changedDirection) {
                ...
                //Just to test, print the coordinates of the Pacer's next move.
                printf("RANDOM WALKER NO. %d'S NEXT MOVE: X = %d, Y = %d\n\n", k + 1, j, i);
            } else {
                printf("RANDOM WALKER NO. %d CHANGED DIRECTION.\n\n", k + 1);
            }
        }
    }
    printf("\n");
