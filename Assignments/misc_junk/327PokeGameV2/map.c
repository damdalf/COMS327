...
void placePokeMart(map_t *m)
{
    ...
    for (i = 0; i < HEIGHT; i++)
    {
        for (j = 0; j < WIDTH; j++)
        {
            ...
            {
                if (m->mapArray[i + 1][j] != PATH && m->mapArray[i + 1][j] != GRASS &&
                    m->mapArray[i + 1][j] != BORDER && m->mapArray[i + 1][j] != 'M' && m->mapArray[i + 1][j] != 'C' &&
                    m->mapArray[i + 1][j + 1] != PATH && m->mapArray[i + 1][j + 1] != GRASS &&
                    m->mapArray[i + 1][j + 1] != BORDER && m->mapArray[i + 1][j + 1] != 'M' &&
                    m->mapArray[i + 1][j + 1] != 'C' && m->mapArray[i + 2][j] != PATH &&
                    m->mapArray[i + 2][j] != GRASS && m->mapArray[i + 2][j] != BORDER && m->mapArray[i + 2][j] != 'M' &&
                    m->mapArray[i + 2][j] != 'C' && m->mapArray[i + 2][j + 1] != PATH &&
                    m->mapArray[i + 2][j + 1] != GRASS && m->mapArray[i + 2][j + 1] != BORDER &&
                    m->mapArray[i + 2][j + 1] != 'M' && m->mapArray[i + 2][j + 1] != 'C')
                {
                    m->mapArray[i + 1][j] = 'M';
                    m->mapArray[i + 2][j] = 'M';
...
void addRandom(map_t *m)
{
    ...
    for (i = 0; i < HEIGHT; i++)
    {
        for (j = 0; j < WIDTH; j++)
        {
            if (m->mapArray[i][j] != '#' && m->mapArray[i][j] != 'M' && m->mapArray[i][j] != 'C' &&
                m->mapArray[i][j] != '%' && m->mapArray[i][j] != '^')
            {
                odds = rand() % 90;
...
int generateSeed()
{
    ...
    while (1)
    {
        ...
        printf("You have entered %d for you seed. Is this correct? (Enter 'y' for yes, and any other character for no): ",
               seed);
        scanf("%c", &input);
        fflush(stdin);
...
void printMap(map_t *m)
{
    ...
    for (i = 0; i < HEIGHT; i++)
    {
        for (j = 0; j < WIDTH; j++)
        {
            if (m->characterArray[i][j] != VACANT && i >= 1 && j >= 1 && i <= 19 && j <= 78)
            {
...
void randomGeneration(map_t *m, const int NUM_TRAINERS)
{
    ...
    initializeHikersAndRivalsCostMaps(m);

    // Print cost maps.
    printPlayerCostMap(m);
    printHikersAndRivalsCostMaps(m);
...
void initializeCostMaps(map_t *m)
{
    ...
    for (i = 0; i < HEIGHT; i++)
    {
        for (j = 0; j < WIDTH; j++)
        {
            ...
            m->hikersCostMap[i][j].visited = false;

            m->rivalsCostMap[i][j].priority = INT_MAX;
            m->rivalsCostMap[i][j].visited = false;
        }
    }
}
        ...
        for (j = 0; j < WIDTH; j++)
        {
            switch (m->mapArray[i][j])
            {
                case BORDER: // inaccessible terrain type for the Hiker.
                    break;
                case TREE:
                    m->hikersCostMap[i][j].priority = 15;
                    break;
                case PATH:
                    m->hikersCostMap[i][j].priority = 10;
                    break;
                ...
                case FLOWER:
                    ...
                    break;
                case PLAYER:
                    m->hikersCostMap[i][j].priority = 0;
                    break;
            }
        }
    }
        ...
        for (j = 0; j < WIDTH; j++)
        {
            switch (m->mapArray[i][j])
            {
                ...
                case TREE: // inaccessible terrain type for the Rival.
                    break;
                case PATH:
                    m->rivalsCostMap[i][j].priority = 10;
                    break;
                ...
                case CENTER: // inaccessible terrain type for the Rival.
                    break;
                case GRASS:
                    m->rivalsCostMap[i][j].priority = 20;
                    break;
                case CLEARING:
                    m->rivalsCostMap[i][j].priority = 10;
                    break;
                ...
                case FLOWER:
                    ...
                    break;
                case PLAYER:
                    m->rivalsCostMap[i][j].priority = 0;
                    break;
            }
        }
    }
...
// Function to print the cost maps for the map's Hikers and Rivals.
void printHikersAndRivalsCostMaps(map_t *m)
{
    ...
    for (i = 0; i < HEIGHT; i++)
    {
        for (j = 0; j < WIDTH; j++)
        {
            if (m->hikersCostMap[i][j].priority == 0)
            {
                printf(B_RED "%2d ", m->hikersCostMap[i][j].priority);
                printf(RESET);
            }
            else if (m->hikersCostMap[i][j].priority == INT_MAX)
            {
                printf("   ");
            }
            else printf("%2d ", m->hikersCostMap[i][j].priority % 100);
        }
    ...
    for (i = 0; i < HEIGHT; i++)
    {
        for (j = 0; j < WIDTH; j++)
        {
            if (m->rivalsCostMap[i][j].priority == 0)
            {
                printf(B_RED "%2d ", m->rivalsCostMap[i][j].priority);
                printf(RESET);
            }
            else if (m->rivalsCostMap[i][j].priority == INT_MAX)
            {
                printf("   ");
            }
            else printf("%2d ", m->rivalsCostMap[i][j].priority % 100);
        }
...
void generatePC(map_t *m)
{
    ...
    m->pc.startingTerrain = m->mapArray[m->pc.xPos][m->pc.yPos];
...
void generateNPC(map_t *m, int k)
{
    ...
    while (!generated)
    {
        // Indices are padded by one to ensure that borders and exits are not used for starting locations.
        for (i = 1; i < HEIGHT - 1; i++)
        {
            for (j = 1; j < WIDTH - 1; j++)
            {
                ...
                {
                    case HIKERS:
                        if (m->hikersCostMap[i][j].priority != INT_MAX)
                        {
                            ...
                            if ((i < HEIGHT - 1 && j < WIDTH - 1 && i > 1 && j > 1 &&
                                 m->characterArray[i][j] != PLAYER && m->characterArray[i][j] != HIKERS &&
                                 m->characterArray[i][j] != RIVALS &&
                                 m->characterArray[i][j] != PACERS && m->characterArray[i][j] != WANDERERS &&
                                 m->characterArray[i][j] != STATIONARIES &&
                                 m->characterArray[i][j] != RANDOM_WALKERS && m->mapArray[i][j] != BORDER &&
                                 m->mapArray[i][j] != MART && m->mapArray[i][j] != CENTER)
                                && (i != m->northExit[0] && i != m->eastExit[0] && i != m->westExit[0] &&
                                    i != m->southExit[0]) && (j != m->northExit[1] &&
                                                              j != m->eastExit[1] && j != m->westExit[1] &&
                                                              j != m->southExit[1]))
                            {
                        ...
                        break;
                    case RIVALS:
                        if (m->rivalsCostMap[i][j].priority != INT_MAX)
                        {
                            // If not, check if the terrain cell is already occupied by another character, or if the current terrain cell is a border or exit.
                            if ((i < HEIGHT - 1 && j < WIDTH - 1 && i > 1 && j > 1 &&
                                 m->characterArray[i][j] != PLAYER && m->characterArray[i][j] != HIKERS &&
                                 ...
                                 m->characterArray[i][j] != STATIONARIES &&
                                 m->characterArray[i][j] != RANDOM_WALKERS && m->mapArray[i][j] != BORDER &&
                                 m->mapArray[i][j] != TREE && m->mapArray[i][j] != ROCK
                                 && m->mapArray[i][j] != MART && m->mapArray[i][j] != CENTER) &&
                                (i != m->northExit[0] && i != m->eastExit[0] && i != m->westExit[0]
                                 && i != m->southExit[0]) &&
                                (j != m->northExit[1] && j != m->eastExit[1] && j != m->westExit[1] &&
                                 j != m->southExit[1]))
                            {
                                // Now it's time to choose a random terrain cell from the available options for the current Trainer's position.
                                odds = rand() % 1000;
                    ...
                    case WANDERERS:
                        ...
                        if ((i < HEIGHT - 1 && j < WIDTH - 1 && i > 1 && j > 1 && m->characterArray[i][j] != PLAYER &&
                             m->characterArray[i][j] != HIKERS && m->characterArray[i][j] != RIVALS &&
                             m->characterArray[i][j] != PACERS && m->characterArray[i][j] != WANDERERS &&
                             m->characterArray[i][j] != STATIONARIES &&
                             m->characterArray[i][j] != RANDOM_WALKERS && m->mapArray[i][j] != BORDER &&
                             m->mapArray[i][j] != TREE && m->mapArray[i][j] != ROCK
                             && m->mapArray[i][j] != MART && m->mapArray[i][j] != CENTER &&
                             m->mapArray[i][j] != FLOWER) &&
                            (i != m->northExit[0] && i != m->eastExit[0] && i != m->westExit[0]
                             && i != m->southExit[0]) &&
                            (j != m->northExit[1] && j != m->eastExit[1] && j != m->westExit[1] &&
                             j != m->southExit[1]))
