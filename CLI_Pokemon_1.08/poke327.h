...
#include "heap.h"
#include "character.h"
// include pokemon.h so that we can create an array of pokemon for Character class
#include "pokemon.h"
...
class Character {
 public:
  ...
  char symbol;
  int next_turn;
  // list of character's six pokemon
  Pokemon p_list[6];
  // number of pokemon actually held
  int num_pokemon;
  // index of active pokemon: naturally set to the PC's first pokemon, randomly set for the NPC.
  int active_index;
  /* 
  While it would be easy to only have a bag for the PC, it is good future proofing to include a bag for all characters, so that if i later choose to add
  better ai I am able to easily allow for a random amount of items for NPCs. Public, so no setter or getter functions needed.
    - index 0: revives
    - index 1: potions
    - index 2: pokeballs
  */
