...
void game_loop(vector<Pokemon_DB> pokemonList, vector<Move_DB> movesList, vector<Pokemon_Move_DB> pokemonMovesList, vector<Pokemon_Species_DB> pokemonSpeciesList,
              vector<Experience_DB> experienceList, vector<Type_Name_DB> typeNamesList, vector<Pokemon_Stat_DB> pokemonStatsList)
{
  character_t *c;
  pair_t d;
  ...
  while (!world.quit)
  {
    ...
    if (c->pc)
    {
      ...
      if (world.cur_map->map[d[dim_y]][d[dim_x]] == ter_grass)
      {
        if (rand() % 10 == 0) generate_pokemon(pokemonList, movesList, pokemonMovesList, pokemonSpeciesList, experienceList, typeNamesList, pokemonStatsList);
      }
...
int main(int argc, char *argv[])
{
  ...
  if (argc == 2)
  {
    ...
    {
      vector<Type_Name_DB> typeNamesList;
      parseTypeNamesList(&typeNamesList);
      printTypeNamesList(&typeNamesList);
    }
  ...
  else
  {
    ...
    // Vectors to store the information parsed from the needed csv files.
    vector<Pokemon_DB> pokemonList;
    vector<Move_DB> movesList;
    vector<Pokemon_Move_DB> pokemonMovesList;
    vector<Pokemon_Species_DB> pokemonSpeciesList;
    ...
    parseAll(&pokemonList, &movesList, &pokemonMovesList, &pokemonSpeciesList, &experienceList, &typeNamesList, &pokemonStatsList);
    ...
    */
    ...
    game_loop(pokemonList, movesList, pokemonMovesList, pokemonSpeciesList, experienceList, typeNamesList, pokemonStatsList);
