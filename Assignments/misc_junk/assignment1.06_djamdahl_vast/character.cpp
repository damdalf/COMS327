...
static void move_hiker_func(Character *c, pair_t dest)
{
  ...
  for (i = base; i < 8 + base; i++)
  {
    ...
    {
      io_battle(c, dynamic_cast<Character*>(&world.pc));
      break;
    }
  }
}
...
{
  // TODO NO LONGER A NEED TO USE CHARACTER, JUST USE THE CLASS NPC
  dest[dim_x] = c->pos[dim_x];
  dest[dim_y] = c->pos[dim_y];

  if (!dynamic_cast<NPC*>(c)->defeated &&
      world.cur_map->cmap[c->pos[dim_y] + dynamic_cast<NPC*>(c)->dir[dim_y]]
                         [c->pos[dim_x] + dynamic_cast<NPC*>(c)->dir[dim_x]] ==
          &world.pc)
  {
    io_battle(c, dynamic_cast<Character*>(&world.pc));
    return;
  ...
  if ((world.cur_map->map[c->pos[dim_y] + dynamic_cast<NPC*>(c)->dir[dim_y]]
                         [c->pos[dim_x] + dynamic_cast<NPC*>(c)->dir[dim_x]] !=
       world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]]) ||
      world.cur_map->cmap[c->pos[dim_y] + dynamic_cast<NPC*>(c)->dir[dim_y]]
                         [c->pos[dim_x] + dynamic_cast<NPC*>(c)->dir[dim_x]])
  {
    dynamic_cast<NPC*>(c)->dir[dim_x] *= -1;
    dynamic_cast<NPC*>(c)->dir[dim_y] *= -1;
  }
  ...
  if ((world.cur_map->map[c->pos[dim_y] + dynamic_cast<NPC*>(c)->dir[dim_y]]
      ...
      !world.cur_map->cmap[c->pos[dim_y] + dynamic_cast<NPC*>(c)->dir[dim_y]]
                          [c->pos[dim_x] + dynamic_cast<NPC*>(c)->dir[dim_x]])
  {
    dest[dim_x] = c->pos[dim_x] + dynamic_cast<NPC*>(c)->dir[dim_x];
    dest[dim_y] = c->pos[dim_y] + dynamic_cast<NPC*>(c)->dir[dim_y];
  }
}
...
// TODO what doesn't it like here?
