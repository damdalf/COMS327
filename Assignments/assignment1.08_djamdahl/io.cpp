...
void io_battle(Character *aggressor, Character *defender)
{
  ...
  dynamic_cast<Npc *>(aggressor) ? npc = dynamic_cast<Npc *>(aggressor) : npc = dynamic_cast<Npc *>(defender);
  ...
  if (world.pc.p_list[0].is_defeated()) {
    io_queue_message("You're lucky all of your Pokemon are knocked out!");
    io_queue_message("");
    }
  else print_trainer_encounter(world.pc, npc);
  if (npc->defeated) {
...
uint32_t move_pc_dir(uint32_t input, pair_t dest)
{
  ...
  if (world.cur_map->cmap[dest[dim_y]][dest[dim_x]]) {
    if (dynamic_cast<Npc *>(world.cur_map->cmap[dest[dim_y]][dest[dim_x]]) &&
      ...
      {
        case 0:
          io_queue_message("You again? Look, you already beat me; I don't want any trouble!");
          break;
        case 1:
          io_queue_message("You're lucky all of my Pokemon are knocked out!");
          break;
        case 2:
          io_queue_message("You beat me, but you won't stand a chance against the gym leader!");
          break;
        case 3:
          // approx max message length
          io_queue_message("My Pokemon are still recovering from our battle earlier... ");
          io_queue_message("You sure are strong!");
          break;
        case 4:
          io_queue_message("I pity the next fool who messes with you... Your looks are deceiving!");
          break;
      }
      return 1;
...
void capitalize_string(string &name)
{
  char last = ' ';
