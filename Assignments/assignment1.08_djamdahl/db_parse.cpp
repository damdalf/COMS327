...
void db_parse(bool print)
{
  ...
  i = (strlen(getenv("HOME")) +
       strlen("/.poke327/pokedex/data/csv/") + 1);
  prefix = (char *) malloc(i);
  strcpy(prefix, getenv("HOME"));
  strcat(prefix, "/.poke327/pokedex/data/csv/");
  ...
  if (!prefix && !stat("/share/cs327", &buf)) {
    prefix = strdup("/share/cs327/pokedex/data/csv/");
  } else if (!prefix) {
  ...
  */
  ...
  // MY CODE: TRYING TO PARSE pokemon_types.csv
