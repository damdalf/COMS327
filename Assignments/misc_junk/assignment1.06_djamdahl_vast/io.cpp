                                     ...
                                     uint32_t count)
{
  ...
  for (i = 0; i < count; i++)
  {
    snprintf(s[i], 40, "%16s %c: %2d %s by %2d %s",
             ...
             dynamic_cast<NPC*>(c[i])->symbol,
             abs(dynamic_cast<NPC*>(c[i])->pos[dim_y] - world.pc.pos[dim_y]),
             ((dynamic_cast<NPC*>(c[i])->pos[dim_y] - world.pc.pos[dim_y]) <= 0 ? "North" : "South"),
             abs(dynamic_cast<NPC*>(c[i])->pos[dim_x] - world.pc.pos[dim_x]),
             ((dynamic_cast<NPC*>(c[i])->pos[dim_x] - world.pc.pos[dim_x]) <= 0 ? "West" : "East"));
    if (count <= 13)
    {
...
void io_battle(Character *aggressor, Character *defender)
{
  ...
  dynamic_cast<NPC*>(npc)->defeated = 1;
  if (dynamic_cast<NPC*>(npc)->ctype == char_hiker || dynamic_cast<NPC*>(npc)->ctype == char_rival)
  {
    dynamic_cast<NPC*>(npc)->mtype = move_wander;
  }
}
...
uint32_t move_pc_dir(uint32_t input, pair_t dest)
{
  ...
  NPC *n = dynamic_cast<NPC *>(world.cur_map->cmap[dest[dim_y]][dest[dim_x]]);
